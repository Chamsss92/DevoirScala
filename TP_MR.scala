...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] = (l,x) match{
    case (Nil,_) => None
    case (a :: tail, 0) => Some(a)
    case (a :: tail, _) => nthA( x-1, tail)
  }


  //TODO define nthB using method of the List collection
  def nthB[T](x: Int, l: List[T]): Option[T] = (l,x) match{
    case (Nil,_) => None
    case (_::_,_) =>
      if (x>0 && x < l.length) {
        Some(l.apply(x))
  ...
  def reverseA[T](list: List[T]): List[T] = {
    def rev[T](list: List[T], acc: List[T]): List[T] = list match {
      case Nil => acc
...
employees.map(e => e.salary).foldLeft(0.0){ (acc, d) => acc+d }
  ...
  users.reverse.foldLeft( List[String]() ){
    (acc, usr) => usr.address :: acc
  }
  ...
  val (sum, len) = values.foldLeft((0.0, 0)){
    (acc,nb) => (acc._1 + nb, acc._2 +1)
  }
  if(len == 0){
    None
  }else{
